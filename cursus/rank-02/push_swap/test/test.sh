...
# check_push_swap "$ARG" 700

for i in {1..10}; do
	# ARG=$(jot -r 200 -2147483648 2147483647 | awk '!seen[$0]++' | head -n 100)
	ARG=$(awk -v seed=$(date +%s%N | cut -b1-13) -v min=-2147483648 -v max=2147483647 'BEGIN { srand(seed); for (i=0; i<100; i++) print int(min + rand() * (max - min + 1)) }')
	check_push_swap "$ARG" 700;
done
...
for i in {1..10}; do
	# ARG=$(jot -r 700 -2147483648 2147483647 | awk '!seen[$0]++' | head -n 500)
	ARG=$(awk -v seed=$(date +%s%N | cut -b1-13) -v min=-2147483648 -v max=2147483647 'BEGIN { srand(seed); for (i=0; i<500; i++) print int(min + rand() * (max - min + 1)) }')
	check_push_swap "$ARG" 5500;
done
