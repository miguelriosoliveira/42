...
./push_swap 2 1 > test/21.received
diff -s test/21.expected test/21.received
...
check_push_swap() {
	...
	local ARG=$(awk -v seed=$RANDOM -v min=-2147483648 -v max=2147483647 -v number_count=$number_count 'BEGIN { srand(seed); for (i=0; i<number_count; i++) print int(min + rand() * (max - min + 1)) }')
	local ARG_ARR=($ARG) # Convert string to an array (split on spaces)
