...
check_push_swap() {
	...
	is_sorted=$(echo "$result" | ./checker_Mac $ARG)

	if [ "$is_sorted" = "OK" ]; then
		received_moves=$(echo "$result" | wc -l | tr -d '[:space:]')

		if [ "$received_moves" -le "$expected_moves" ]; then
			echo "✅ Numbers properly sorted in $received_moves moves (max $expected_moves)"
		else
			echo "❌ Numbers properly sorted, but in $received_moves moves (max $expected_moves)"
		fi
	else
